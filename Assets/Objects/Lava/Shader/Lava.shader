Shader "Flowing Lava Surface/Lava" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			{
				float4 noise = tex2D(_NoiseTex, i.tex);
				float2 t1 = i.tex + float2( 1.5, -1.5) * _Time.y * 0.02;
				float2 t2 = i.tex + float2(-0.5,  2.0) * _Time.y * _LavaFlowSpeed;
				...
				float p = tex2D(_NoiseTex, t1 * _NoiseTile).a;

				float4 color = tex2D(_LavaTex, t2 * _LavaTile);
				float4 lava = color * (float4(p, p, p, p) * _LavaBright) + (color * color - _LavaDarkOffset);
				
				fixed4 mask = tex2D(_MaskTex, i.tex2.xy);
				fixed4 albedo = tex2D(_AlbedoTex, i.tex2.zw);
